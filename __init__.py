...
def register(ctx):
    ...
    from . import social_nlp

    # ---- Config -----------------------------------------------------------
    # PluginContext has no .config attribute — read config directly
    from hermes_cli.config import load_config, cfg_get
    _all_config = load_config()
    _social_cfg = cfg_get(_all_config, "plugins", "social_tracking", default={})
    if not isinstance(_social_cfg, dict):
        _social_cfg = {}
    cfg = SocialTrackingConfig(**_social_cfg)
    ...
    def _make_tool_proxy(tool_name: str) -> Callable[..., Any]:
        """Wrap ctx.dispatch_tool to return parsed JSON results."""

        def _proxy(**tool_kwargs):
            raw = ctx.dispatch_tool(tool_name, tool_kwargs)
            if isinstance(raw, str):
                try:
                    return json.loads(raw)
                except json.JSONDecodeError:
                    logger.debug("%s returned non-JSON payload: %s", tool_name, raw)
                    return raw
            return raw
    ...
    tool_registry = {
        "neural_remember": _make_tool_proxy("neural_remember"),
        "neural_recall": _make_tool_proxy("neural_recall"),
        "neural_think": _make_tool_proxy("neural_think"),
        "neural_graph": _make_tool_proxy("neural_graph"),
    }

    mem = MemoryClient(tool_registry)
    ...
    def _llm_call(prompt: str, model: Optional[str]) -> str:
        ...
        llm = ctx.llm
        resolved_model = model or cfg.tom_model or "default"
        return llm.chat(prompt, model=resolved_model)
    ...
    def _run_social_nlp(
        ...
        """Run LLM-based social extraction with graceful fallback."""
        try:
            return social_nlp.extract_social_signals(
    ...
    def post_llm_call(assistant_response: str, user_message: str, **_kw):
        ...
        # ──────────────────────────────────────────────────────────────
        # NLP-powered social extraction (primary) with regex fallback
        # ──────────────────────────────────────────────────────────────
        nlp = _run_social_nlp(user_message, assistant_response, all_pids)

        # 1. SENTIMENT → trust (NLP primary, keyword fallback)
        if nlp.get("sentiments"):
            social_nlp._apply_sentiment_trust(db, trust_mgr, nlp["sentiments"], all_pids)
        else:
            _auto_sentiment_trust(db, trust_mgr, user_message, all_pids)

        # 2. COMMITMENTS (NLP primary, regex fallback)
        nlp_committed = False
        if nlp.get("commitments"):
            count = social_nlp._apply_commitments(db, nlp["commitments"], all_pids)
            nlp_committed = count > 0

        if not nlp_committed:
            # Regex fallback — original detection paths
            ...
            if cid:
                logger.info("Auto-detected commitment (regex): assistant→user: %s", cid)
            ...
            if user_cid:
                logger.info("Auto-detected commitment (regex): user→assistant: %s", user_cid)
        ...
        # 3. RELATIONSHIPS (NLP primary, regex fallback)
        nlp_rels = False
        if nlp.get("relationships"):
            count = social_nlp._apply_relationships(db, nlp["relationships"], all_pids)
            nlp_rels = count > 0
        ...
        # 4. INTERACTION CLASSIFICATION (NLP primary, keyword fallback)
        if nlp.get("interaction_class"):
            kind = social_nlp._classify(nlp["interaction_class"])
        else:
...
# =======================================================================
    ...
    def social_remember(args_dict: dict = None, **_extras) -> dict:
        args = args_dict or {}
        content = args.get("content", "")
        person_name = args.get("person_name", "")
        category = args.get("category", "social")
    ...
    def social_query(args_dict: dict = None, **_extras) -> dict:
        args = args_dict or {}
        person_name = args.get("person_name", "")
        query_type = args.get("query_type", "trust")
        if query_type == "trust":
    ...
    def social_commit(args_dict: dict = None, **_extras) -> dict:
        args = args_dict or {}
        description = args.get("description", "")
        promisee_name = args.get("promisee_name", "user")
    ...
    def social_fulfill(args_dict: dict = None, **_extras) -> dict:
        args = args_dict or {}
        commitment_id = args.get("commitment_id", "")
        fulfiller_name = args.get("fulfiller_name", "assistant")
    ...
    ctx.register_tool("social_remember", "social", {
    ...
    ctx.register_tool("social_query", "social", {
    ...
    ctx.register_tool("social_commit", "social", {
    ...
    ctx.register_tool("social_fulfill", "social", {
...
def _resolve_mazemaker_db(ctx) -> str:
    ...
    from hermes_cli.config import load_config, cfg_get
    _all_config = load_config()
    return cfg_get(_all_config, "plugins", "social_tracking", "db_path",
                   default=os.path.expanduser("~/.hermes/social_tracking.db"))
